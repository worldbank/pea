...
program pea_table8, rclass
	...
	* See results
	...
	collect notes 2: `"Note: The Gini index is a measure of inequality ranging from 0 (perfect equality) to 100 (perfect inequality). The Theil Index measures the distance of the welfare distribution from perfect equality. The Palma (Kuznets) ratio measures the top 10 percent income share relative to the bottom 40 percent share. The Atkinson index is an inequality measure with a weighting parameter which measures aversion to inequality. The Sen index combines the number of poor, their poverty depth and the Gini index. The Watts index is the average log-point differences from the poverty line. Welfare ratios are presented for different percentiles; For example, p10/p50 refers to the ratio of consumption or income between those who are at the 10th percentile and those who are at the 50th percentile of the welfare distribution. The bottom 20% share of incomes indicates the share of total income or consumption held by the bottom 20% of the welfare distribution."'
	_pea_tbtformat
