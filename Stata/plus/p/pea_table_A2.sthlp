{smcl}
{hline}
{title:table_A2} — Generate detailed poverty and welfare analysis tables with specified parameters.

{hline}
{title:Syntax}
    {bf:table_A2} [{it:if}] [{it:in}] [{it:weight}], {cmdab:Country(string)} 
...
{title:Description}
    {pstd} {cmd:table_A2} generates a detailed table that provides poverty and welfare statistics based on both 
...
{title:Remarks}
    {pstd} The {cmd:table_A2} command performs poverty and welfare analysis based on a range of national and 
