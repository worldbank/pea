...
program pea_figure10c, rclass
	version 18.0
	syntax [if] [in] [aw pw fw], [Country(string) Year(varname numeric) BENCHmark(string) ONEWelfare(varname numeric) YRange(string) scheme(string) palette(string) save(string) excel(string) within(integer 3) PPPyear(integer 2021)]	
	...
	* Rest
	...
	cap graph close	
	//Export data
	export excel country_code year pg ln_gdp_pc group using "`excelout2'" , sheet("Figure10c", modify) cell(O11) keepcellfmt firstrow(variables) nolabel
