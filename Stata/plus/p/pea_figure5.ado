...
program pea_figure5, rclass
	version 18.0
	syntax [if] [in] [aw pw fw], [ONEWelfare(varname numeric) ONELine(varname numeric) spells(string) Year(varname numeric) urban(varname numeric) CORE LINESORTED setting(string) excel(string) save(string) palette(string)]
	...
	qui {	
		...
		* See results
		...
		graph bar value if decomp=="Huppi-Ravallion", over(subind) over(spell) asyvar legend(rows(1) size(small) position(6)) ytitle("Total change in poverty in percentage point", size(medium)) name(gr_decomp, replace) scheme(white_tableau) title("Huppi-Ravallion decomposition", size(medium)) blabel(bar, position(center) format(%9.2f)) ///
		note("Source: World Bank calculations using survey data accessed through the Global Monitoring Database", size(small))
		*caption("Note: The Huppi-Ravallion decomposition shows how much changes in total poverty can be attributed to" "income or consumption growth and redistribution using `note'", size(small))
