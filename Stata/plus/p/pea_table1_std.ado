...
//Table 1. Core poverty indicators, tables 1 and 8 (long annex) with standard errors
...
program pea_table1_std, rclass
	...
	qui {
		...
		if "`svy'"~="" {
			...
			if _rc~=0 {
				...
				exit `=_rc'
				//or
				//svyset [w= `wvar'],  singleunit(certainty)
			}
			else {
		...
		if "`std'"=="" local std inside
			...
			local std = lower("`std'")
			if "`std'"~="inside" & "`std'"~="below" & "`std'"~="right" {
				noi dis "Wrong option for std(). Available options: inside, below, right"
				exit 198
			}
		}
	...
	qui foreach dat of local datalist {
		...
		drop if `single'==1
		svy: mean _fgt* _prosgap_`pppwelfare' _vulpov_`onewelfare'_`oneline' if `touse'
	...
	//merge with STD data
	merge 1:1 _varname `year' using `data2a'
	drop _merge
	...
	replace std = std*100 if _varname2=="vulpov"|_varname2=="fgt0"|_varname2=="fgt1"|_varname2=="fgt2"
	...
	qui if "`std'"=="right" {
		//wide-form
		table (indicatorlbl subind) (`year') ,statistic(mean value) nototal nformat(%20.2f) missing
		table (indicatorlbl subind) (`year') if std!=. ,statistic(mean std) nototal nformat(%20.2f) missing append
	...
	*collect style cell cell_type[column-header], warn shading( background(lightskyblue) )
	collect style cell cell_type[corner], shading( background(lightskyblue) )
	*collect style cell cell_type[column-header row-header corner], font(, bold) shading( background(seashell) )
	collect style cell cell_type[column-header corner], font(, bold) shading( background(seashell) )
	*collect style cell cell_type[row-header], warn shading( background(lightskyblue) )
