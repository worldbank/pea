...
{p 4 4 2} {opt Country(string)} specifies the country code for which the indicators are to be generated.
  
{p 4 4 2} {opt Year(varname numeric)} specifies the year variable for the analysis.
  
{p 4 4 2} {opt CORE setting(string)} specifies custom settings, such as region or data type.
...
{p 4 4 2}
{opt pea_table9} imports multiple Scorecard Vision Indicator files, extracts the relevant country and region data, and generates a table that includes:
...
{p 4 4 2}
pea_table9, Country("GHA") Year(year) excel("output_table9.xlsx")
