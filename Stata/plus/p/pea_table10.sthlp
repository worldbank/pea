...
{p 4 4 2}
{opt pea table10} calculates and generates tables of poverty and equity-related indicators for poverty analysis. It extracts data for specified countries and years, and produces a detailed table summarizing poverty and inequality measures, including poverty headcount ratio, income distribution, and equity indices. Results can be exported to Excel or saved in a specified file format.

{title:Options}

{p 4 4 2} {opt Country(string)} specifies the country code for which the poverty indicators are to be generated.
 
{p 4 4 2} {opt Year(varname numeric)} specifies the year variable for the analysis.

{p 4 4 2} {opt Indicator(varname)} specifies the specific indicator(s) to include in the analysis, such as poverty headcount or Gini index.
 
{p 4 4 2} {opt excel(string)} specifies the file path for exporting results to Excel. If omitted, results are saved to a temporary file.
...
{p 4 4 2}
{opt pea_table10} imports relevant poverty and equity datasets, extracts the country-specific and region-specific data for the selected year, and generates a table with:
